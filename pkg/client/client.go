...
// TiggClient is the main client that holds both configuration and methods
...
// Config struct to initialize client; passes configuration when creating a new client
type Config struct {
	ClientKey string
	SecretKey string
	Namespace string
...
func (c *TiggClient) AddHeaders(req *http.Request) {
	...
	req.Header.Set("X-API-Key", c.ClientKey)
	req.Header.Set("X-Nonce", fmt.Sprintf("%d", time.Now().UnixNano()))
	req.Header.Set("X-Timestamp", fmt.Sprintf("%d", time.Now().Unix()))
	req.Header.Set("namespace", c.Namespace)
}
